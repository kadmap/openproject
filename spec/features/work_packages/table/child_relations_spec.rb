...
RSpec.describe "Work Package table child relations", :js, with_ee: %i[work_package_query_relation_columns] do
  ...
  let(:children_column_id) { "childrenOfType#{type.id}" }
  let(:children_column_all) { "childRelations" }
  ...
  describe "with child relation columns allowed by the enterprise token" do
    it "displays expandable child relation columns" do
      ...
      columns.add("Children of #{type.name}")
      ...
      expect(parent_row).to have_css(".#{children_column_id} .wp-table--relation-count", text: "2")
      expect(parent_row).to have_css(".#{children_column_all} .wp-table--relation-count", text: "2")

      # Expect count for child1 in both columns to be not rendered
      expect(child1_row).to have_no_css(".#{children_column_id} .wp-table--relation-count")
      expect(child1_row).to have_no_css(".#{children_column_all} .wp-table--relation-count")

      # Expand first column
      parent_row.find(".#{children_column_id} .wp-table--relation-indicator").click
      expect(page).to have_css(".__children-expanded-from-#{parent.id}", count: 2)
      related_row = page.first(".__children-expanded-from-#{parent.id}")
      expect(related_row).to have_css("td.wp-table--relation-cell-td", text: "Child")

      # Collapse
      parent_row.find(".#{children_column_id} .wp-table--relation-indicator").click
      expect(page).to have_no_css(".__children-expanded-from-#{parent.id}")

      # Expand second column
      parent_row.find(".#{children_column_all} .wp-table--relation-indicator").click
      expect(page).to have_css(".__children-expanded-from-#{parent.id}", count: 2)
      related_row = page.first(".__children-expanded-from-#{parent.id}")
      expect(related_row).to have_css(".wp-table--relation-cell-td", text: child1.type)
    end
  end
  ...
  describe "with child relation columns disallowed by the enterprise token", with_ee: false do
    it "has no child relation columns available for selection" do
      ...
      columns.expect_column_not_available "children relations"
      columns.expect_column_not_available "Children of #{type.name}"
    end
  end
end
